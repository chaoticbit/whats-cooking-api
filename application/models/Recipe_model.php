...
class Recipe_model extends CI_Model {   
    public function newrecipe($data) {
        ...
        $query = $this->db->query("select recipes.srno as recipe_id, recipes.title, recipes.description, recipes.cover_imagepath, recipes.prep_time, recipes.cooking_time, recipes.calorie_intake, recipes.servings, recipes.spicy, recipes.food_group, recipes.cid, recipes.uid, recipes.timestamp, CONCAT(useraccounts.fname,' ',useraccounts.lname) as fullname, userprofile.profile_imagepath, useraccounts.username, cuisines.name as cname FROM recipes, useraccounts, cuisines, userprofile where recipes.uid = " . (int)$data['user_id']. " AND recipes.uid = userprofile.uid AND userprofile.uid = useraccounts.srno AND recipes.cid=cuisines.srno order by timestamp DESC LIMIT 1");
        if($query->num_rows() > 0) {
    ...
    public function load_more($data) {
        ...
        $query = $this->db->query("select recipes.srno as recipe_id, recipes.title, recipes.description, recipes.cover_imagepath, recipes.prep_time, recipes.cooking_time, recipes.calorie_intake, recipes.servings, recipes.spicy, recipes.food_group, recipes.cid, recipes.uid, recipes.timestamp, CONCAT(useraccounts.fname,' ',useraccounts.lname) as fullname, userprofile.profile_imagepath, useraccounts.username, cuisines.name as cname, ratings.rating FROM recipes, useraccounts, userprofile, cuisines, cuisine_user, ratings where (recipes.food_group <= userprofile.food_group) AND recipes.uid = userprofile.uid AND userprofile.uid = useraccounts.srno AND recipes.srno=ratings.rid AND recipes.cid=cuisines.srno AND recipes.cid=cuisine_user.cid AND cuisine_user.uid=" . $user_id . " and recipes.timestamp < '$last_ts' order by timestamp desc limit 5");      
        if($query->num_rows() > 0) {             
            $result = $query->result_array();                                                                      
            for($i=0;$i<count($result);$i++) {
                $result[$i]['time_elapsed'] = time_elapsed($result[$i]['timestamp']);
    ...
    public function per_category_recipes($data) {
        ...
        $query = $this->db->query("select distinct recipes.srno as recipe_id, recipes.title, recipes.description, recipes.cover_imagepath, recipes.prep_time, recipes.cooking_time, recipes.calorie_intake, recipes.servings, recipes.spicy, recipes.food_group, recipes.cid, recipes.uid, recipes.timestamp, CONCAT(useraccounts.fname,' ',useraccounts.lname) as fullname, userprofile.profile_imagepath, useraccounts.username, cuisines.name as cname, ratings.rating FROM recipes, useraccounts, userprofile, cuisines, cuisine_user, ratings where cuisines.srno = " . $cid . " AND recipes.uid = userprofile.uid AND userprofile.uid = useraccounts.srno AND recipes.srno=ratings.rid AND recipes.cid=cuisines.srno AND recipes.cid=cuisine_user.cid order by timestamp DESC LIMIT 10");
        if($query->num_rows() > 0) { 
            $result = $query->result_array();      
            for($i=0;$i<count($result);$i++) {
                if($result[$i]['cover_imagepath'] != '') 
    ...
    public function per_tag_recipes($data) {
        ...
        if($query->num_rows() > 0) {
            $query1 = $this->db->query("select distinct recipes.srno as recipe_id, recipes.title, recipes.description, recipes.cover_imagepath, recipes.prep_time, recipes.calorie_intake, recipes.cooking_time, recipes.servings, recipes.spicy, recipes.food_group, recipes.cid, recipes.uid, recipes.timestamp, CONCAT(useraccounts.fname,' ',useraccounts.lname) as fullname, useraccounts.username, userprofile.profile_imagepath, cuisines.name as cname, ratings.rating FROM recipes, useraccounts, userprofile, cuisines, cuisine_user, ratings, recipe_tags where recipe_tags.rid = recipes.srno AND recipe_tags.name = '" . $tag . "' AND recipes.uid = userprofile.uid AND userprofile.uid = useraccounts.srno AND recipes.srno=ratings.rid AND recipes.cid=cuisines.srno AND recipes.cid=cuisine_user.cid order by recipes.timestamp DESC LIMIT 10");
